...
class TwoColoursAssignmentArena(Arena):
    ...
    def __init__(self, objects=None, wall_markers=False):
        ...
        def place_token_circle(radius, number_offset=0, angle_offset=0.25 * pi,
            ...
            gx = [1.574,1.854,-2.129,1.43,0.671,-0.265]
            gy = [-1.157,1.882,0.146,2.474,-1.884,0.895]
            sx = [-0.599,1.093,-2.355,0.32,-0.448,0.739]
            sy = [0.309,-0.426,0.286,2.488,-0.469,-2.244]
            ...
            for i in range(TOKENS_PER_CIRCLE):
                ...
                if token_type == SilverToken:
                    token.location = (sx[i], sy[i])
                else:
                    token.location = (gx[i], gy[i])
